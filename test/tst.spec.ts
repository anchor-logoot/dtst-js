...
import { DTst, DTstNode, CompareResult, cmpResult } from '../src/index'

class DummyNode extends DTstNode<DummyNode> {
...
describe('DTst (Differential Ternary Search Tree)', () => {
  let b: DTst<DummyNode>
  beforeEach(() => {
    b = new DTst<DummyNode>()
  })
  afterEach(() => {
    b.selfTest()
  })
  ...
  describe('additions', () => {
    it('should add to BST root', () => {
      ...
      b.add(n1)
      expect(b.tst_root).to.be.equal(n1)
      expect(b.tst_root.value).to.be.equal(5)
    })
    it('gt/lt should add to correct side of root', () => {
      ...
      expect(b.tst_root.left_node).to.be.equal(n2)
      expect(n2.value).to.be.equal(-3)
      expect(b.tst_root.right_node).to.be.equal(n3)
    ...
    it('should create/reorder equal_nodes containing equal values', () => {
      ...
      expect(b.tst_root).to.be.equal(n2)
      expect(n2.value).to.be.equal(5)
      expect(b.tst_root.equal_nodes[0]).to.be.equal(n1)
      expect(n1.value).to.be.equal(0)
      expect(b.tst_root.equal_nodes[1]).to.be.equal(n3)
      expect(n3.value).to.be.equal(0)
      expect(b.tst_root.equal_nodes[2]).to.be.equal(n4)
    ...
    it('should insert equal node to equal_nodes', () => {
      ...
      expect(b.tst_root).to.be.equal(n3)
    ...
    it('should form a complete tree', () => {
      ...
      expect(b.tst_root).to.be.equal(n5)
      expect(n5.value).to.be.equal(5)

      expect(b.tst_root.left_node).to.be.equal(n3)
      expect(n3.value).to.be.equal(-2)
      expect(b.tst_root.left_node.left_node).to.be.equal(n2)
      expect(n2.value).to.be.equal(-1)
      expect(b.tst_root.left_node.right_node).to.be.equal(n4)
      expect(n4.value).to.be.equal(1)

      expect(b.tst_root.right_node).to.be.equal(n7)
      expect(n7.value).to.be.equal(2)
      expect(b.tst_root.right_node.left_node).to.be.equal(n6)
      expect(n6.value).to.be.equal(-1)
      expect(b.tst_root.right_node.right_node).to.be.equal(n8)
  ...
  describe('node replacement', () => {
    ...
    it('should remove node from previous location', () => {
      const { n2, n3, n4, n5, n6, n7, n8 } = constructFullTree()
      n5.replaceWith(n4, (p) => b.tst_root = p)
      expect(b.tst_root).to.be.equal(n4)
      ...
      expect(b.tst_root.left_node.left_node).to.be.equal(n2)
      expect(n2.value).to.be.equal(-1)
      expect(b.tst_root.left_node.right_node).to.be.equal(undefined)
  ...
  describe('addSpaceBefore', () => {
    it('should properly rearrange', () => {
      ...
      b.add(n3)
      b.add(n4)
      n2.addSpaceBefore(-1, (np: DummyNode) => (b.tst_root = np))
    ...
    it('should offset newly added equal node', () => {
      ...
      b.add(n2)
      b.add(nn)
      n0.addSpaceBefore(1, (np: DummyNode) => (b.tst_root = np))
    ...
    it('position of re-added nodes from removal is conserved', () => {
      const n0 = new DummyNode(0, 0)
      const n1 = new DummyNode(5, 5)
      const n2 = new DummyNode(3, 3)
      const n3 = new DummyNode(6, 6)
      const n4 = new DummyNode(11, 11)
      b.add(n0)
      b.add(n1)
      ...
      b.add(n4)
      n1.addSpaceBefore(-2)
      expect(n0.n).to.be.equal(0)
      expect(n1.n).to.be.equal(3)
      expect(n2.n).to.be.equal(3)
      expect(n3.n).to.be.equal(4)
      expect(n4.n).to.be.equal(9)
    })
    it('equal node transplated child retains equal nodes', () => {
      const n0 = new DummyNode(0, 0)
      const n1 = new DummyNode(5, 5)
      const n2 = new DummyNode(3, 3)
      const n3 = new DummyNode(6, 6)
      const n4 = new DummyNode(6, 7)
      const n5 = new DummyNode(6, 8)
      const n6 = new DummyNode(11, 11)
      b.add(n0)
      b.add(n1)
      b.add(n2)
      b.add(n3)
      ...
      expect(n2.n).to.be.equal(3)
      expect(n3.n).to.be.equal(4)
      expect(n4.n).to.be.equal(4)
      expect(n5.n).to.be.equal(4)
      expect(n6.n).to.be.equal(9)
    })
    describe('positive offset test', () => {
      const pt = (i: number) => {
        const { n2, n3, n4, n5, n6, n7, n8 } = constructFullTree()
        const array = [n2, n3, n4, n5, n6, n7, n8]
        array[i].addSpaceBefore(1, (np: DummyNode) => (b.tst_root = np))
  ...
  describe('removals', () => {
    ...
    it('remove root', () => {
      const { n2, n3, n4, n5, n6, n7, n8 } = constructFullTree()
      n5.remove((p) => (b.tst_root = p))
      expect(b.tst_root).to.be.equal(n6)
      expect(b.tst_root.value).to.be.equal(6)
    ...
    it('removal replaces with equal node', () => {
      ...
      b.add(n5)
      b.add(n6)
      n4.remove((n) => (b.tst_root = n))
      expect(b.tst_root).to.be.equal(n5)
      expect(n5.equal_nodes.length).to.be.equal(1)
      expect(n5.equal_nodes[0]).to.be.equal(n6)
    })
    it('position of children of removed nodes is conserved', () => {
      const n0 = new DummyNode(0, 0)
      const n1 = new DummyNode(3, 5)
      const n2 = new DummyNode(2, 3)
      const n3 = new DummyNode(4, 6)
      const n4 = new DummyNode(9, 11)
      b.add(n0)
      b.add(n1)
      b.add(n2)
      b.add(n3)
